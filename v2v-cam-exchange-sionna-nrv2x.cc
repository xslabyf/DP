 ...
 */
#include "ns3/digital-twin-client-app.h"
...
void receiveCAM(asn1cpp::Seq<CAM> cam, Address from, StationID_t my_stationID, StationType_t my_StationType, SignalInfo phy_info)
{
  ...
  uint64_t tx_id = cam->header.stationId;
  uint64_t rx_id = my_stationID;
...
int main (int argc, char *argv[])
{
  ...
  // === SEND SUMO MAP FILE TO PYTHON SERVER ===
  {
      std::string mapPath = sumo_folder + "map.net.xml";   // ➜ použijeme rovnakú folder premennú

      std::ifstream file(mapPath);
      if (!file.is_open())
      {
          std::cerr << "[NS3] ERROR: Cannot open SUMO map file: " << mapPath << std::endl;
      }
      else
      {
          ...
          std::string cmd =
              ...
              "-d '" + payload + "' "
              "http://127.0.0.1:6001/upload_map > /dev/null 2>&1";

          std::cout << "[NS3] Sending SUMO MAP to Digital Twin server ("
                    << mapContent.size() << " bytes)" << std::endl;
  ...
  STARTUP_FCN setupNewWifiNode = [&] (std::string vehicleID, TraciClient::StationTypeTraCI_t stationType) -> Ptr<Node>
  {
    ...
    Ptr<DigitalTwinClientApp> dtApp = CreateObject<DigitalTwinClientApp> ();
    dtApp->SetAttribute ("RemoteAddress",
                         AddressValue (InetSocketAddress (Ipv4Address ("127.0.0.1"), 5005)));
    dtApp->SetAttribute ("RemotePort", UintegerValue (5005));
    dtApp->SetAttribute ("Interval", TimeValue (MilliSeconds (100))); // 10 Hz

    // TraCI client a SUMO id (veh + stationId)
    std::string sumoVehId = "veh" + std::to_string(vehID);
    dtApp->SetTraci (sumoClient, sumoVehId);

    includedNode->AddApplication (dtApp);
    dtApp->SetStartTime (Seconds (0.1));
    dtApp->SetStopTime (Seconds (simTime));
